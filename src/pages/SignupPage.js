// src/pages/SignupPage.js - FINAL VERSION WITH STARRY SKY BACKGROUND
import React, { useState } from 'react';
import { Link, useNavigate } from 'react-router-dom';
import { createUserWithEmailAndPassword, signInWithPopup, signInWithPhoneNumber } from 'firebase/auth';
    ...
    <div className="min-h-screen">
      ...
      <div className="relative z-10 flex items-center justify-center min-h-screen pt-20 px-4">
        ...
        <div className="bg-black bg-opacity-50 backdrop-blur-sm p-8 rounded-lg shadow-2xl shadow-white/20 w-full max-w-md">
          <h2 className="text-3xl font-bold text-center text-animated-gradient mb-8">Create an Account</h2>
          {error && <p className="bg-red-500 text-white text-center p-3 rounded-md mb-4">{error}</p>}
          
          {view === 'otp' ? (
            <form onSubmit={verifyOtp}>
              <div className="mb-4"><label className="block text-gray-400 mb-2">Enter OTP sent to +91{phone}</label><input type="text" value={otp} onChange={(e) => setOtp(e.target.value)} className="w-full bg-gray-800 text-white px-4 py-2 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-600" required /></div>
              <button type="submit" className="w-full bg-animated-gradient text-white font-bold py-3 rounded-lg">Verify & Sign Up</button>
              <button type="button" onClick={() => { setView('phone'); setError(''); }} className="w-full mt-4 text-purple-400 hover:underline">Back to Phone Sign Up</button>
            </form>
          ) : view === 'email' ? (
            <form onSubmit={handleEmailSignup}>
              ...
              <button type="submit" className="w-full bg-animated-gradient text-white font-bold py-3 rounded-lg">Sign Up</button>
              <button type="button" onClick={() => setView('phone')} className="w-full mt-4 text-purple-400 hover:underline">Sign Up with Phone instead</button>
            </form>
          ) : (
            <form onSubmit={handlePhoneSignIn}>
              ...
              <button type="button" onClick={() => setView('email')} className="w-full mt-4 text-purple-400 hover:underline">Sign Up with Email instead</button>
            </form>
          )}
